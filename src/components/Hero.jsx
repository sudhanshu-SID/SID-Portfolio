...
export default function Hero() {
  ...
  const yRaw = useTransform(scrollY, (val) => val * 0.3);
  // Subtle parallax on the whole hero layer; disabled under reduced-motion.
    ...
    >
      <motion.div style={{ y }} className="absolute inset-0 w-full h-full will-change-transform">
        ...
        <div className="absolute inset-0 z-0">
          <img
            src={getCloudinaryUrl('hero-full', 'image', { width: 1600 })}
            alt=""
            aria-hidden="true"
            loading="eager"
        ...
        <div className="absolute inset-0 z-10 hidden sm:flex items-center pointer-events-none">
          <div className="w-full overflow-hidden">
            {/* CSS compositor-driven marquee (see .hero-marquee-track in index.css).
                Two identical strips + translate3d(-50%) = seamless loop, off the main
                thread. Reduced-motion is handled in CSS. */}
            <div className="hero-marquee-track items-center whitespace-nowrap">
              <MarqueeStrip />
              <MarqueeStrip />
            </div>
          </div>
        ...
        {/* Vignette edges — static radial gradient (cheaper to paint than a large
            blurred inset box-shadow, which repaints on any layer change) */}
        <div
